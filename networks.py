...
class AdaINGen(nn.Module):
    ...
    def __init__(self, input_dim, params, fp16):
        ...
        #self.enc_content1 = ContentEncoder(n_downsample, n_res, 1, dim, 'in', activ, pad_type=pad_type, dropout=dropout, tanh=tanh, res_type='basic')
    ...
    def encode1(self, images):
        # encode an image to its content and style codes
        content = self.enc_content1(images)
        return content
...
class StyleEncoder(nn.Module):
    def __init__(self, n_downsample, input_dim, dim, style_dim, norm, activ, pad_type):
        ...
        self.model += [nn.AdaptiveMaxPool2d((4,1))] # global average pooling
       # self.model += [Conv2dBlock(dim, 128)]
        #self.model += [Conv2dBlock(128, 64)]
        #self.model += [LinearBlock(dim,8)]
        
        self.model = nn.Sequential(*self.model)
        self.avgpool = nn.AdaptiveAvgPool2d((1,1))
        self.classifier = ClassBlock(256, 8, 0.9)
        self.output_dim = dim

    def forward(self, x):
        #pdb.set_trace()
        x = self.model(x)
        f = x.view(x.size(0),x.size(1)*4)
...
#         modulelist = list(self.model.children())
...
class ContentEncoder(nn.Module):
    def __init__(self, n_downsample, n_res, input_dim, dim, norm, activ, pad_type, dropout, tanh=False, res_type='basic'):
        ...
        #self.model += [LinearBlock(128, 64)]
        #self.model += [LinearBlock(64, 6)]
        self.model = nn.Sequential(*self.model)
        self.output_dim = dim
...
#             x1 = l(x1)
#         for l in modulelist[6:7]:
#             x2 = l(x1)
#             x2 = x2.view(8,-1)
#         for l in modulelist[7:]:
#             x2 = l(x2)
#         return x1,x2
        return self.model(x)
