...
from dataloader import ReIDFolder
import torch
import os
import math
...
def get_all_data_loaders(conf):
    ...
    bone_folder_market = conf['bone_folder_market']
    bone_folder_duke = conf['bone_folder_duke']
    mask_folder_market = conf['mask_folder_market']
    mask_folder_duke = conf['mask_folder_duke']
    annotations_file_path = conf['annotations_file_path']
    if 'new_size' in conf:
    ...
    if 'data_root' in conf:
        train_loader_a = get_data_loader_folder(os.path.join(conf['data_root_duke'], 'train_all'), batch_size, bone_folder_duke, mask_folder_duke, True,
                                              new_size_a, height, width, num_workers, True)
       # train_loader_c = get_data_loader_folder(os.path.join(conf['data_root_duke'], 'train_all'), batch_size, bone_folder_duke, annotations_file_path, True,
                                       #       new_size_a, height, width, num_workers, True)
      #  test_loader_a = get_data_loader_folder(os.path.join(conf['data_root_duke'], 'query'), batch_size, False,
                                       #      new_size_a, height, width, num_workers, False)
        train_loader_b = get_data_loader_folder(os.path.join(conf['data_root_market'], 'train_all'), batch_size, bone_folder_market, mask_folder_market, True,
                                              new_size_b, height, width, num_workers, True)
        #train_loader_d = get_data_loader_folder(os.path.join(conf['data_root_market'], 'train_all'), batch_size, bone_folder_market, annotations_file_path, True,
                                  #            new_size_b, height, width, num_workers, True)
      #  test_loader_b = get_data_loader_folder(os.path.join(conf['data_root_market'], 'query'), batch_size, False,
...
def get_data_loader_list(root, file_list, batch_size, train, new_size=None,
                           height=128, width=64, num_workers=4, crop=True):
...
def get_data_loader_folder(input_folder, batch_size, bone_folder, mask_folder, train, new_size=None,
                           height=128, width=64, num_workers=4, crop=True):
    transform_list = [transforms.ToTensor(),
                      transforms.Normalize((0.485, 0.456, 0.406),
                                           (0.229, 0.224, 0.225))]
    ...
    transform_list = [transforms.RandomHorizontalFlip()] + transform_list if train else transform_list
    transform = transforms.Compose(transform_list)
    dataset = ReIDFolder(input_folder, bone_folder,mask_folder,transform=transform)
    loader = DataLoader(dataset=dataset, batch_size=batch_size, shuffle=True, drop_last=True, num_workers=num_workers)
    return loader
...
def __write_images(image_outputs, display_image_num, file_name):
    #image_outputs = [image_outputs.expand(-1, 3, -1, -1)]
